...
3. If the task is not yet complete, choose the appropriate integration agent (Gmail, Linear, or Slack) based on the user's request and the current progress.
4. If the instructions are unclear, or you are unsure which integration agent to choose, always call transfer_to_summary_agent as a fallback.
5. Remember, transfer_to_summary_agent must be called under two conditions:
...
6. Do not pass any arguments when calling the transfer functions; they do not accept any parameters.
""",
    tools=[
